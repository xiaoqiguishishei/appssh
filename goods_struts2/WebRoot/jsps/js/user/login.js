$(function() {
	...
	$("#submit").hover(
		function() {
			$("#submit").attr("src", "/goods_struts2/images/login2.jpg");
		},
		function() {
			$("#submit").attr("src", "/goods_struts2/images/login1.jpg");
		}
	);
...
function validateVerifyCode() {
	...
	if(!value) {//非空校验
		...
		$.ajax({
			cache: false,
			async: false,//同步, 意味着必须等到success方法执行结束才能执行下面 return的代码
			type: "POST",
			dataType: "json",
			data: {method: "validateVerifycode", verifycode: value},
			url: "/goods_struts2/user/validateVerifycodeAction",
